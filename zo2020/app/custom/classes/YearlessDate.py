...
@deconstructible
class CustomYearlessDate(object):
    ...
    def _validate(self):
        ...
        try:
            ...
            raise Exception('Cannot create CustomYearlessDate object with a month value of %d.' % self.month)

        if self.day < 1 or self.day > month_days:
             raise Exception('Cannot create CustomYearlessDate object - invalid day value %d for month %s.' % (self.day, self.month_name))
    ...
    def __eq__(self, other):
        if not isinstance(other, CustomYearlessDate):
            return False
